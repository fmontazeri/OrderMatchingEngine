...
public class AddOrder_EventTests : AddOrder_StateTests
{
    ...
    public override async Task AddOrder_Should_Place_Order_After_Receiving_The_First_Order(OrderSide orderSide)
    {
        await base.AddOrder_Should_Place_Order_After_Receiving_The_First_Order(orderSide);
    ...
    [InlineData(OrderSide.Buy)]
    [InlineData(OrderSide.Sell)]
    public override async Task AddOrder_Should_Place_A_New_Order_When_Queue_Have_Some_Others(OrderSide orderSide)
    {
        await base.AddOrder_Should_Place_A_New_Order_When_Queue_Have_Some_Others(orderSide);
        AssertOrderPlacedEvent();
    }
    ...
    public override async Task AddOrder_IsMatched_When_The_BuyOrder_And_SellOrder_AreEqual(OrderSide queuedOrderSide,
        OrderSide incomingOrderSide)
    {
        await base.AddOrder_IsMatched_When_The_BuyOrder_And_SellOrder_AreEqual(queuedOrderSide, incomingOrderSide);
    ...
    [Fact]
    public override async Task AddOrder_Should_Not_Be_Matched_When_Both_Sell_And_Buy_Orders_Are_From_The_Same_Customer()
    {
        await base.AddOrder_Should_Not_Be_Matched_When_Both_Sell_And_Buy_Orders_Are_From_The_Same_Customer();
        CurrentOrderBook.GetPublishedEvents().Should().HaveCount(1);
        CurrentOrderBook.AssertEvent(new OrderPlacedDomainEvent(IncomingOrder));
    }

    [Fact]
    public override async Task AddOrder_Should_Not_Matched_When_The_Both_Sell_And_Buy_Instrument_Are_Not_The_Same()
    {
        await base.AddOrder_Should_Not_Matched_When_The_Both_Sell_And_Buy_Instrument_Are_Not_The_Same();
